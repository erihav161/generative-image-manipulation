...
class CSSDataset(BaseDataset):
    ...
    def get_1st_training_query(self):
        ...
        k= np.random.randint(0, len(mod['from']))
        self.last_from = mod['from'][j]
        self.last_mod = [i]
        return mod['from'][j], i, mod['to'][j], mod['to'][k]

    def get_2nd_training_query(self):
        modid, new_to = random.choice(self.imgid2modtarget[self.last_from])
        _, wrong = random.choice(self.imgid2modtarget[self.last_from])
        while modid in self.last_mod:
            modid, new_to = random.choice(self.imgid2modtarget[self.last_from])
            _, wrong = random.choice(self.imgid2modtarget[self.last_from])
        self.last_mod += [modid]
        # mod = self.mods[modid]
        return self.last_from, modid, new_to, wrong

    def generate_random_query_target(self):
        try:
            if len(self.last_mod) < 2:
                img1id, modid, img2id, wrongid = self.get_2nd_training_query()
            else:
                img1id, modid, img2id, wrongid = self.get_1st_training_query()
        except:
            img1id, modid, img2id, wrongid = self.get_1st_training_query()
        ...
        out['wrong_img_id'] = wrongid
        out['wrong_img_data'] = self.get_img(wrongid)
