...
class generator(nn.Module):
    ...
    def forward(self, x1, sen_embed):
        ...
        # coord_tensor = self.coord_tensor.repeat(batch_size, 1, 1, 1)
...
class discriminator(nn.Module):
    def __init__(self):
        ...
        self.phisencoder = nn.Sequential( # input is (2*lstm_hidden) x 1 x 1
            nn.ConvTranspose2d(2*self.lstm_hidden, self.ndf*8, 4, 1, 0, bias=False), # (ndf*8) x 4 x 4
            nn.BatchNorm2d(self.ndf*8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose2d(self.ndf*8, self.ndf*8, 4, 2, 1, bias=False),  # (ndf*8) x 8 x 8
            nn.BatchNorm2d(self.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True)
        ) # output is # (ndf*8) x 8 x 8
